...
import TherapistRecorder from "../components/TherapistRecorder";
...
export default function TherapistScreen({
  ...
  const [recordedExercise, setRecordedExercise] = useState<ReferenceExercise | null>(() =>
    loadReferenceExercise(),
  );
  ...
  const selectableExercises = EXERCISES.filter(
    (exercise) =>
      exercise.id !== "custom" ||
      recordedExercise ||
      draft.steps.some((step) => step.exerciseId === "custom"),
  );
  const addableExercises = EXERCISES.filter(
    (exercise) => exercise.id !== "custom" || recordedExercise,
  );

  function displayExerciseName(exerciseId: ExerciseId) {
    return exerciseId === "custom" && recordedExercise
        ...
        <section className="card" aria-label="Plan editor">
                    ...
                    >
                      {selectableExercises.map((exercise) => (
                        <option key={exercise.id} value={exercise.id}>
                          {displayExerciseName(exercise.id)}
                        </option>
                      ))}
                    </select>
                  </label>

                  <div className="stepper" role="group" aria-label={`Reps for ${displayExerciseName(step.exerciseId)}`}>
                    <button
                      type="button"
          ...
          <div>
            ...
            <div className="picker">
              ...
              >
                ...
                {recordedExercise ? `Re-record ${recordedExercise.name}` : "Record custom exercise"}
              </button>
            </div>
          </div>
          ...
          {showRecorder && (
            <TherapistRecorder
              ...
              onPublished={() => setRecordedExercise(loadReferenceExercise())}
            />
          )}
